...
from PySide6.QtCore import Qt, Signal, QObject, QSize, QPropertyAnimation, QEasingCurve, QTimer, QUrl
...
from PySide6.QtWidgets import (
    ...
    QButtonGroup, QGraphicsOpacityEffect, QLineEdit, QListWidget, QListWidgetItem
)
...
VIDEO_INDEX = OUTPUT / 'videos.json'
ASSETS = ROOT / 'assets'
...
class BrowserAutomation:
    ...
    def login_cookie(self, cookie_text):
        if self._thread and self._thread.is_alive():
            self.signals.status.emit(f'{self.provider.upper()} browser đang chạy.')
            return
        self._stop.clear()
        self._thread = threading.Thread(target=self._cookie_loop, args=(cookie_text,), daemon=True, name=f'{self.provider}-cookie')
        self._thread.start()

    def _parse_cookie_text(self, text, domain):
        text = (text or '').strip()
        if not text:
            return []
        try:
    ...
    def _cookie_loop(self, cookie_text):
        profile, url = self._profile_url()
        domain = '.meta.ai' if self.provider == 'meta' else '.google.com'
        try:
            sync_playwright=self._import_pw()
            with sync_playwright() as pw:
                self._pw=pw
                self.signals.status.emit(f'Đang nạp cookie {self.provider.upper()}...')
                context=self._launch_browser(pw, profile)
                self._context=context
                page=context.pages[0] if context.pages else context.new_page()
                page.set_default_timeout(8000)
                cookies=self._parse_cookie_text(cookie_text,domain)
                if not cookies:
                    raise RuntimeError('Cookie trống hoặc không đúng định dạng.')
                ...
                if state != 'logged_in':
                    raise RuntimeError('Cookie không được xác nhận hoạt động. Meta/Google vẫn yêu cầu đăng nhập.')
                self.signals.login_state.emit(self.provider,True)
                self.signals.status.emit(f'🟢 Cookie {self.provider.upper()} hoạt động — đã đăng nhập.')
                time.sleep(1.5)
        except Exception as e:
            log_event(f'{self.provider} cookie error: {e}','ERROR')
            self.signals.login_state.emit(self.provider,False)
            self.signals.error.emit(str(e))
        finally:
            self._context=None; self._page=None; self._pw=None
    ...
    def _upload_images(self, page, images):
        if not images:
            return 0
        # Meta frequently changes whether the file input is directly visible.
        # Try direct inputs first, then use a real file chooser triggered by the
        # visible upload/add-photo control.
        ...
        try:
            ...
            for i in range(n - 1, -1, -1):
                el = inputs.nth(i)
                try:
                    el.set_input_files(images, timeout=5000)
                    return len(images)
                except Exception:
                    continue
        except Exception:
        ...
        upload_names = [
            'Add photos', 'Add photo', 'Upload photos', 'Upload photo',
            'Add image', 'Add images', 'Upload image', 'Upload images',
            'Photo', 'Photos', 'Image', 'Images', 'Ảnh', 'Thêm ảnh', 'Tải ảnh lên'
        ]
        for name in upload_names:
            locs = [
                page.get_by_role('button', name=re.compile(re.escape(name), re.I)),
                page.get_by_label(re.compile(re.escape(name), re.I)),
    ...
    def _find_send_button(self, page, prompt_el=None):
        patterns = [r'send', r'submit', r'generate', r'create', r'tạo', r'gửi', r'send prompt']
        selectors = [
            ...
            try:
                ...
                for i in range(min(loc.count(),150)):
                    ...
                    for attr in ('aria-label','title','data-testid','name'):
                        try: bits.append((el.get_attribute(attr) or '').lower())
                        except Exception: pass
                    try: bits.append((el.inner_text(timeout=250) or '').strip().lower())
                    except Exception: pass
                    label=' '.join(bits)
                ...
                pass
        # Prefer a button inside the same form/container as the prompt editor.
        if prompt_el is not None:
            try:
                near = prompt_el.locator('xpath=ancestor::form[1]//button | xpath=ancestor::*[self::div or self::section][1]//button')
                for i in range(min(near.count(),40)):
                    el=near.nth(i)
    ...
    def _send_prompt(self, page, prompt_el):
        for _ in range(20):
            btn=self._find_send_button(page,prompt_el)
            if btn:
                try:
                    btn.scroll_into_view_if_needed(timeout=1500)
                    btn.click(timeout=3500,force=False)
                    return True
                except Exception:
                    try:
        ...
        # Last-resort keyboard submit only after button discovery has failed.
        try:
            prompt_el.click(timeout=1000)
            prompt_el.press('Enter')
            return True
        except Exception:
            return False

    def _find_video_result(self, page):
        # Do not treat every <video> as success. Require a playable source or a
        # nearby download action.
        videos = page.locator('video:visible')
        try:
            for i in range(videos.count()):
                v=videos.nth(i)
                src=v.get_attribute('src') or ''
                current=v.get_attribute('currentSrc') or ''
                if src or current:
                    return v
        except Exception:
            pass
    ...
    def generate_flow_image(self, prompt, images, output_dir, count=1, ratio='16:9'):
        threading.Thread(
            target=self._generate_flow_image,
            args=(prompt, images, output_dir, count, ratio),
            daemon=True,
            name='flow-image-generation'
        ).start()

    def _flow_click(self, page, labels, timeout=2500):
        for label in labels:
    ...
    def _flow_prompt(self, page):
        # Flow currently uses a prompt box inside the project editor. Prefer
        # editable textareas/contenteditables and ignore hidden marketing inputs.
        for sel in ['textarea:visible', '[contenteditable="true"]:visible', 'input[type="text"]:visible']:
            try:
                loc = page.locator(sel)
    ...
    def _flow_upload(self, page, images):
        if not images:
            return 0
        inputs = page.locator('input[type=file]')
        try:
            for i in range(inputs.count() - 1, -1, -1):
                try:
                    inputs.nth(i).set_input_files(images, timeout=5000)
                    return len(images)
                except Exception:
                    pass
        except Exception:
            pass
        # Official Flow UI supports Add Image / Upload Image. Use a real
        # file chooser so no fake upload state is reported.
        labels = ['Add Image', 'Add image', 'Upload Image', 'Upload image', 'Add images', 'Upload images']
        for label in labels:
            rx = re.compile(re.escape(label), re.I)
            for loc in [page.get_by_role('button', name=rx), page.get_by_label(rx), page.get_by_text(rx)]:
                try:
                    if not loc.count() or not loc.first.is_visible():
                        continue
                    with page.expect_file_chooser(timeout=5000) as info:
                        loc.first.click(timeout=3000)
                    info.value.set_files(images)
                    return len(images)
                except Exception:
                    continue
        return 0

    def _flow_image_sources(self, page):
        sources = set()
        try:
            imgs = page.locator('img:visible')
            for i in range(min(imgs.count(), 500)):
                el = imgs.nth(i)
                try:
    ...
    def _flow_find_new_image(self, page, before):
        # Generated assets appear as new image elements. Require a meaningful
        # rendered size to avoid counting tiny icons/avatars as output.
        try:
            imgs = page.locator('img:visible')
            for i in range(min(imgs.count(), 500) - 1, -1, -1):
                el = imgs.nth(i)
                try:
                    src = (el.get_attribute('src') or '').strip()
                    if not src or src in before or src.startswith('data:image/svg'):
                        continue
                    box = el.bounding_box()
    ...
    def _download_flow_asset(self, page, image_el, out):
        # Official Flow supports Ctrl+D for Download in Grid/Edit view.
        # First try the explicit download action, then the documented shortcut.
        download_labels = ['Download', 'Tải xuống', 'Save to device', 'Save image']
        for label in download_labels:
            rx = re.compile(re.escape(label), re.I)
            locs = [page.get_by_role('button', name=rx), page.get_by_label(rx), page.get_by_text(rx)]
            for loc in locs:
                try:
                    if loc.count() and loc.first.is_visible() and loc.first.is_enabled():
    ...
    def _generate_flow_image(self, prompt, images, output_dir, count, ratio):
        try:
            sync_playwright = self._import_pw()
            output_dir = Path(output_dir)
            output_dir.mkdir(parents=True, exist_ok=True)
            with sync_playwright() as pw:
                self.signals.status.emit('Đang mở Google Flow...')
                context = self._launch_browser(pw, FLOW_PROFILE)
                page = context.pages[0] if context.pages else context.new_page()
                page.set_default_timeout(8000)
                page.goto(FLOW_URL, wait_until='commit', timeout=20000)
                try:
                    page.wait_for_load_state('domcontentloaded', timeout=10000)
                except Exception:
                    pass
                if self._detect_login_state(page) != 'logged_in':
                    raise RuntimeError('Google Flow chưa đăng nhập hoặc tài khoản chưa được Flow cho phép.')

                self.signals.status.emit('Đang mở project Google Flow...')
                prompt_el = self._flow_prompt(page)
                if not prompt_el:
                    # The desktop Flow UI requires a project. Prefer an existing
                    # project; if none is open, create a new project using the
                    # documented + New / New project action.
                    if not self._flow_click(page, ['+ New', 'New project', 'New Project', 'Dự án mới', '+ Project mới'], 3500):
                        raise RuntimeError('Không tìm thấy project hoặc nút tạo project mới trong Google Flow.')
                    page.wait_for_timeout(1200)
                    prompt_el = self._flow_prompt(page)
                if not prompt_el:
                    raise RuntimeError('Không tìm thấy ô prompt của Google Flow.')

                # Standard prompt box: choose Image generation when the control
                # exposes the current generation type.
                self._flow_click(page, ['Image', 'Images', 'Ảnh'], 1800)
                page.wait_for_timeout(400)
                if images and self._flow_upload(page, images) != len(images):
                    raise RuntimeError('Không thể upload ảnh tham chiếu vào Google Flow.')
                before = self._flow_image_sources(page)
                prompt_el.fill(prompt)

                self.signals.status.emit('Đang bấm Generate Image trên Google Flow...')
                if not self._flow_click(page, ['Generate Image', 'Generate image', 'Tạo Image', 'Tạo ảnh'], 3500):
                    # Enter is used only after the real Generate Image control was
                    # not discoverable, and success is still verified below.
                    try:
                        prompt_el.press('Enter')
                    except Exception:
                        raise RuntimeError('Không tìm thấy nút Generate Image của Google Flow.')

                self.signals.status.emit('🟡 Google Flow đang tạo ảnh thật...')
                deadline = time.time() + 10 * 60
                new_image = None
                while time.time() < deadline:
                    page.wait_for_timeout(2500)
                    new_image = self._flow_find_new_image(page, before)
                    if new_image:
                        break
                    # Flow can add the asset without changing the visible src list
                    # immediately; refreshing the grid is an official recovery step.
                    try:
                        ...
                        pass
                if not new_image:
                    raise RuntimeError('Google Flow chưa trả về ảnh mới. Không đánh dấu thành công.')

                saved = 0
                for n in range(max(1, int(count))):
                    out = output_dir / f'flow_image_{datetime.now().strftime("%Y%m%d_%H%M%S")}_{n+1}.png'
                    if self._download_flow_asset(page, new_image, out):
                        saved += 1
                    else:
                        break
                if saved == 0:
                    raise RuntimeError('Đã thấy ảnh mới trên Google Flow nhưng chưa tải được file ảnh thật.')
                context.close()
                self.signals.done.emit(str(out))
                self.signals.status.emit(f'🟢 Google Flow đã tải {saved} ảnh thật.')
        except Exception as e:
            log_event(f'Flow image error: {e}', 'ERROR')
            self.signals.error.emit(str(e))
    ...
    def _generate_meta_video(self, prompt, images, output_dir):
        try:
            ...
            with sync_playwright() as pw:
                ...
                self.signals.status.emit('Đang bấm Gửi/Tạo video...')
                if not self._send_prompt(page, prompt_el):
                    raise RuntimeError('Không tìm thấy hoặc không thể bấm nút Gửi/Tạo video của Meta AI.')
                self.signals.status.emit('🟡 Meta AI đã nhận prompt — đang chờ video thật...')
                deadline = time.time() + 15 * 60
                video = None
                while time.time() < deadline:
                    page.wait_for_timeout(2500)
                    video = self._find_video_result(page)
                    if video:
                        break
                    # A real download link can appear before the video tag is hydrated.
                    try:
                        links=page.locator('a[download]:visible, a[href*=".mp4"]:visible')
                        if links.count():
                            video=links.first
                            break
                    except Exception:
                ...
                saved = False
                # First try a real browser download action.
                for label in ['Download', 'Download video', 'Tải xuống', 'Tải video']:
                    try:
                        ...
                        continue
                # Then try an actual download link.
                if not saved:
                    try:
                        link=page.locator('a[download]:visible, a[href*=".mp4"]:visible').first
                        if link.count():
                            with page.expect_download(timeout=12000) as dl_info:
                        ...
                        pass
                # Finally request a normal http(s) media URL. Blob URLs are intentionally
                # not treated as a fake success.
                if not saved:
                    src=video.get_attribute('src') or video.get_attribute('currentSrc') or ''
                    if src.startswith(('http://','https://')):
                        resp=context.request.get(src, timeout=30000)
                        if resp.ok and len(resp.body()) > 1024:
                            out.write_bytes(resp.body())
                            saved=True
                if not saved or not out.exists() or out.stat().st_size < 1024:
                    raise RuntimeError('Đã thấy video nhưng chưa tải được file video thật.')
                context.close()
                try:
                    entries=[]
                    if VIDEO_INDEX.exists(): entries=json.loads(VIDEO_INDEX.read_text(encoding='utf-8'))
                    entries=[x for x in entries if isinstance(x,dict) and Path(x.get('path','')).exists()]
                    entries.insert(0,{'path':str(out),'created_at':now(),'provider':'Meta AI','prompt':prompt})
                    VIDEO_INDEX.write_text(json.dumps(entries[:500],ensure_ascii=False,indent=2),encoding='utf-8')
                except Exception as idx_e:
                    log_event(f'Không ghi được danh sách video: {idx_e}','WARN')
                self.signals.done.emit(str(out))
...
class LoginPage(FadePage):
    def __init__(self, app):
        ...
        for provider, name, desc in [('meta','Meta AI','Session riêng cho Meta AI.'), ('flow','Google Flow','Session riêng cho Google Flow.')]:
            ...
            cookie=QLineEdit(); cookie.setPlaceholderText('Dán cookie/session của bạn (JSON hoặc name=value;...)')
            cookie.setEchoMode(QLineEdit.Password)
            test=QPushButton('🔐 Kiểm tra cookie')
            test.clicked.connect(lambda _,p=provider,e=cookie:self.app.login_cookie(p,e.text()))
            cookie_row.addWidget(cookie,1); cookie_row.addWidget(test); c.addLayout(cookie_row)
...
class MetaVideoPage(FadePage):
    ...
    def on_done(self,path):
        self.progress.hide(); self.status.setText('🟢 Đã nhận video thật: '+Path(path).name); self.generate.setEnabled(True); self.preview.setText('🎬  '+Path(path).name+'\n\nĐã lưu trong output. Vào “Video đã tạo” để xem.'); self.app.video_results.refresh(); self.app.show_page(self.app.page_index('Video đã tạo')); Toast(self,'Tạo video thành công')


class VideoResultsPage(FadePage):
    def __init__(self, app):
        super().__init__(app.animations, app.settings['animation_level']); self.app=app
        lay=QVBoxLayout(self); lay.setContentsMargins(24,20,24,24); lay.setSpacing(12)
        t=QLabel('🎬  VIDEO ĐÃ TẠO'); t.setObjectName('pageTitle'); lay.addWidget(t)
        self.list=QListWidget(); self.list.setMinimumHeight(150); self.list.itemClicked.connect(self.play_selected); lay.addWidget(self.list)
        self.player=QMediaPlayer(self); self.audio=QAudioOutput(self); self.player.setAudioOutput(self.audio)
        self.video=QVideoWidget(); self.video.setMinimumHeight(420); self.video.setObjectName('videoPreview'); lay.addWidget(self.video,1); self.player.setVideoOutput(self.video)
        row=QHBoxLayout(); self.open_btn=QPushButton('📂 Mở thư mục'); self.open_btn.clicked.connect(self.open_folder); self.play_btn=QPushButton('▶ Phát'); self.play_btn.clicked.connect(self.play_current); self.stop_btn=QPushButton('⏹ Dừng'); self.stop_btn.clicked.connect(self.player.stop); self.del_btn=QPushButton('🗑 Xóa file'); self.del_btn.clicked.connect(self.delete_selected); row.addWidget(self.open_btn); row.addWidget(self.play_btn); row.addWidget(self.stop_btn); row.addWidget(self.del_btn); row.addStretch(); lay.addLayout(row)
        self.refresh()

    def refresh(self):
        self.list.clear(); out=Path(self.app.settings.get('output_dir',str(OUTPUT))); out.mkdir(parents=True,exist_ok=True)
        files=sorted(out.glob('*.mp4'), key=lambda p:p.stat().st_mtime, reverse=True)
        for f in files:
            item=QListWidgetItem(f'🎬 {f.name}   •   {f.stat().st_size//1024} KB'); item.setData(Qt.UserRole,str(f)); self.list.addItem(item)
        if not files:
            self.list.addItem('Chưa có video nào được tạo.')
    ...
    def delete_selected(self):
        item=self.list.currentItem(); path=item.data(Qt.UserRole) if item else None
        if path and Path(path).exists():
            try: Path(path).unlink(); self.refresh(); Toast(self.app,'Đã xóa video')
            except Exception as e: QMessageBox.warning(self,'Xóa video',str(e))
...
class LogPage(FadePage):
    def __init__(self, app):
        super().__init__(app.animations, app.settings['animation_level']); self.app=app
        lay=QVBoxLayout(self); lay.setContentsMargins(24,20,24,24); lay.setSpacing(12)
        h=QHBoxLayout(); t=QLabel('📋  LOG CHI TIẾT'); t.setObjectName('pageTitle'); h.addWidget(t); h.addStretch(); r=QPushButton('🔄 Làm mới'); r.clicked.connect(self.refresh); h.addWidget(r); c=QPushButton('🧹 Xóa log'); c.clicked.connect(self.clear); h.addWidget(c); lay.addLayout(h)
        self.text=QTextEdit(); self.text.setReadOnly(True); lay.addWidget(self.text,1); self.refresh()
        self.timer=QTimer(self); self.timer.timeout.connect(self.refresh); self.timer.start(1500)

    def refresh(self):
        try:
            self.text.setPlainText(LOG_FILE.read_text(encoding='utf-8') if LOG_FILE.exists() else 'Chưa có log.')
            self.text.moveCursor(self.text.textCursor().End)
        except Exception as e: self.text.setPlainText(f'Không đọc được log: {e}')

    def clear(self):
        try: LOG_FILE.write_text('',encoding='utf-8'); self.refresh(); Toast(self.app,'Đã xóa log')
        except Exception as e: QMessageBox.warning(self,'Log',str(e))


class FlowImagePage(FadePage):
    def __init__(self, app):
        super().__init__(app.animations, app.settings['animation_level']); self.app=app; self.images=[]
        lay=QVBoxLayout(self); lay.setContentsMargins(24,20,24,24); lay.setSpacing(12)
        t=QLabel('🖼️  GOOGLE FLOW / TẠO ẢNH'); t.setObjectName('pageTitle'); lay.addWidget(t)
        d=QLabel('Google Flow chỉ dùng cho tạo ảnh. Tool mở đúng profile Flow, nhập prompt, bấm Generate Image và chỉ báo thành công khi tải được file ảnh thật.'); d.setObjectName('muted'); d.setWordWrap(True); lay.addWidget(d)
        row=QHBoxLayout(); left=QVBoxLayout(); right=QVBoxLayout(); row.addLayout(left,3); row.addLayout(right,2); lay.addLayout(row,1)
        refs=QFrame(); refs.setObjectName('card'); rc=QVBoxLayout(refs); hh=QHBoxLayout(); hh.addWidget(QLabel('ẢNH THAM CHIẾU')); hh.addStretch(); add=QPushButton('+ Thêm ảnh'); add.clicked.connect(self.add_images); hh.addWidget(add); rc.addLayout(hh)
        self.scroll=QScrollArea(); self.scroll.setWidgetResizable(True); self.scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff); self.thumbhost=QWidget(); self.thumbs=QGridLayout(self.thumbhost); self.thumbs.setAlignment(Qt.AlignLeft|Qt.AlignTop); self.scroll.setWidget(self.thumbhost); self.scroll.setMinimumHeight(150); rc.addWidget(self.scroll); left.addWidget(refs)
        pc=QFrame(); pc.setObjectName('card'); pl=QVBoxLayout(pc); pl.addWidget(QLabel('PROMPT')); self.prompt=QTextEdit(); self.prompt.setPlaceholderText('Nhập prompt tạo ảnh...'); self.prompt.setMinimumHeight(190); pl.addWidget(self.prompt); left.addWidget(pc)
        opt=QFrame(); opt.setObjectName('card'); og=QGridLayout(opt)
        og.addWidget(QLabel('Số ảnh'),0,0); self.count=QSpinBox(); self.count.setRange(1,4); self.count.setValue(1); og.addWidget(self.count,1,0)
        og.addWidget(QLabel('Tỷ lệ'),0,1); self.ratio=QComboBox(); self.ratio.addItems(['16:9','9:16','1:1','4:3','3:4']); og.addWidget(self.ratio,1,1); left.addWidget(opt)
        st=QFrame(); st.setObjectName('card'); sl=QVBoxLayout(st); sl.addWidget(QLabel('TRẠNG THÁI')); self.status=QLabel('🟡 Chưa bắt đầu'); self.status.setObjectName('bigStatus'); sl.addWidget(self.status); self.progress=QProgressBar(); self.progress.setRange(0,0); self.progress.hide(); sl.addWidget(self.progress); right.addWidget(st)
        pv=QFrame(); pv.setObjectName('previewCard'); pvl=QVBoxLayout(pv); pvl.addWidget(QLabel('KẾT QUẢ')); self.preview=QLabel('Ảnh thật sẽ xuất hiện sau khi Google Flow hoàn tất và tải file.'); self.preview.setAlignment(Qt.AlignCenter); self.preview.setMinimumHeight(350); self.preview.setObjectName('preview'); pvl.addWidget(self.preview,1); right.addWidget(pv,1)
        self.generate=QPushButton('✨  TẠO ẢNH TRÊN FLOW'); self.generate.setObjectName('primaryBig'); self.generate.clicked.connect(self.generate_image); right.addWidget(self.generate)
        openb=QPushButton('📂 Mở thư mục output'); openb.clicked.connect(self.open_output); right.addWidget(openb)

    def add_images(self):
        files,_=QFileDialog.getOpenFileNames(self,'Chọn ảnh','',['Images (*.png *.jpg *.jpeg *.webp *.bmp)'])
        for f in files: self.add_one(f)
    def add_one(self,path):
        if path not in self.images: self.images.append(path); self.rebuild_thumbs()
    def rebuild_thumbs(self):
        while self.thumbs.count():
            item=self.thumbs.takeAt(0); w=item.widget()
            if w: w.deleteLater()
    ...
    def generate_image(self):
        prompt=self.prompt.toPlainText().strip()
        if not prompt: QMessageBox.warning(self,'Thiếu prompt','Hãy nhập prompt tạo ảnh.'); return
        if not self.app.flow_logged: QMessageBox.warning(self,'Chưa đăng nhập','Hãy vào Đăng nhập → Google Flow trước.'); return
        self.generate.setEnabled(False); self.progress.show(); self.status.setText('🟡 Đang xử lý trên Google Flow...')
        worker=BrowserAutomation('flow'); worker.signals.status.connect(self.status.setText); worker.signals.error.connect(self.on_error); worker.signals.done.connect(self.on_done); worker.generate_flow_image(prompt,self.images,Path(self.app.settings.get('output_dir',str(OUTPUT))),self.count.value(),self.ratio.currentText()); self._worker=worker
    def on_error(self,e):
        self.progress.hide(); self.status.setText('🔴 '+e); self.generate.setEnabled(True); Toast(self,'Google Flow: '+e)
    def on_done(self,path):
        self.progress.hide(); self.status.setText('🟢 Đã tải ảnh thật: '+Path(path).name); self.generate.setEnabled(True)
        pix=QPixmap(path)
        ...
        Toast(self,'Google Flow tạo ảnh thành công')
...
class DashboardPage(FadePage):
    def __init__(self, app):
        ...
        self.cards=[]
        for i,(title,value,desc) in enumerate([('Meta AI','🔴 Chưa đăng nhập','Browser profile riêng'),('Google Flow','🔴 Chưa đăng nhập','Browser profile riêng'),('Output','0','File đã lưu'),('Animation','ON','Có thể đổi trong Cài đặt')]):
    ...
    def refresh(self, meta, flow, animations):
        self.cards[0].setText('🟢 Đã đăng nhập' if meta else '🔴 Chưa đăng nhập'); self.cards[1].setText('🟢 Đã đăng nhập' if flow else '🔴 Chưa đăng nhập'); self.cards[2].setText(str(len(list(Path(self.app.settings.get('output_dir', str(OUTPUT))).glob('*'))))); self.cards[3].setText('ON' if animations else 'OFF')
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setAttribute(Qt.WA_TranslucentBackground, False); self.settings=load_settings(); self.animations=self.settings['animations']; self.meta_logged=False; self.flow_logged=False; self.login_workers={}
        ...
        logo=QLabel(); logo.setObjectName('logo'); logo.setAlignment(Qt.AlignCenter); logo.setPixmap(QPixmap(str(ASSETS/'hn38.png')).scaled(190,150,Qt.KeepAspectRatio,Qt.SmoothTransformation)); sl.addWidget(logo); brand=QLabel(APP_NAME+'\nAI VIDEO STUDIO'); brand.setAlignment(Qt.AlignCenter); brand.setObjectName('brand'); sl.addWidget(brand); sl.addSpacing(18)
    ...
    def build_nav(self):
        while self.nav.count():
            ...
            if w: w.deleteLater()
        self.buttons=[]
        groups=[('WORKSPACE',[('🏠','Dashboard'),('🔐','Đăng nhập')]),('GOOGLE FLOW',[('🖼️','Flow / Tạo Ảnh')]),('META AI',[('🎬','Tạo Video'),('📁','Video đã tạo')]),('SYSTEM',[('📋','Log chi tiết'),('⚙️','Cài đặt')])]
        self.page_defs=[]
        ...
        for name,_ in self.page_defs:
            ...
            elif name=='Flow / Tạo Ảnh': p=FlowImagePage(self)
            elif name=='Tạo Video': p=MetaVideoPage(self)
            elif name=='Video đã tạo': p=VideoResultsPage(self)
            elif name=='Log chi tiết': p=LogPage(self)
            elif name=='Cài đặt': p=SettingsPage(self)
            else: p=PlaceholderPage(self,name,'Khu vực này đã được chuẩn bị để tích hợp tác vụ thật.')
            self.stack.addWidget(p); self.pages.append(p)
        self.login_page=self.pages[1]; self.video_page=self.pages[self.page_index('Tạo Video')]; self.video_results=self.pages[self.page_index('Video đã tạo')]; self.dashboard=self.pages[0]

    def page_index(self,name):
        for i,(n,_) in enumerate(self.page_defs):
            if n==name: return i
        return -1
    ...
    def login_cookie(self,provider,cookie_text):
        if not cookie_text.strip():
            QMessageBox.warning(self,'Cookie','Hãy dán cookie trước khi kiểm tra.')
            return
        worker=self.login_workers.get(provider)
        if worker and worker._thread and worker._thread.is_alive():
            try: worker.stop()
        ...
        worker=BrowserAutomation(provider); self.login_workers[provider]=worker
        worker.signals.status.connect(self.on_browser_status); worker.signals.error.connect(lambda e:self.browser_error(e)); worker.signals.login_state.connect(self.on_login_state); worker.login_cookie(cookie_text)
    ...
    def apply_theme(self):
        ...
        #sidebar{{background:rgba(9,21,38,235);border-right:1px solid {t['border']}}}
        ...
        #card,#statCard,#previewCard{{background:rgba(12,27,46,220);border:1px solid {t['border']};border-radius:14px}}
        ...
        QTextEdit,QComboBox,QSpinBox,QLineEdit{{background:rgba(10,23,40,210);border:1px solid {t['border']};border-radius:9px;padding:9px;color:{t['text']}}}
